...
cor(output$Age, output$PredAge, method = "pearson") # 0.9756371

write.csv(output, "BorutaPy_Age_enet_10FCV_fl.csv", row.names = F)

###### Saving the model
saveRDS(Top2_Model, file = "Model_Top2B_enet_fl.RData")
# Loading the model
...
dim(SelectCpGs)
dim(PredictAge)
cor(SelectCpGs$OriginalAge, SelectCpGs$PredictAge, method = "pearson") #Checking correlation # 0.938872
...
write.csv(Age_Compare, 'Merged_fl_Diseased_AgeCompare.csv', row.names = F)
...
ggplot(Age_Compare, aes(OriginalAge, PredictAge)) +
  ...
  ggtitle("Age Comparison Plot - fl [r = 0.938872]") +
