...
df = read.csv("CpG_236BorutaPy_MergedBoth_schiz.csv",header = T)
...
ggplot(df_long, aes(x = CpG, y = value, color = Type)) +
  geom_boxplot() +
  labs(x = "\nCpG", y = "Methylation Value\n", title = "Comparing methylation in healthy vs diseased: Schizophrenia") +
...
ggplot(df_long, aes(x = CpG, y = value, color = Type)) +
  geom_boxplot() +
  labs(x = "\nCpG", y = "Methylation Value\n", title = "Comparing methylation in healthy vs diseased: Schizophrenia (Top 30 CpG)") +
  scale_color_manual(values = c("blue", "red"), name = "Type") +
  theme_minimal() +
  theme(
