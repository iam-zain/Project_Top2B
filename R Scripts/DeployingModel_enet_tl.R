...
cor(output$Age, output$PredAge, method = "pearson") # 0.7929228

write.csv(output, "BorutaPy_Age_enet_10FCV_fl.csv", row.names = F)

###### Saving the model
saveRDS(Top2_Model, file = "Model_Top2B_enet_tl.RData")
# Loading the model
...
dim(SelectCpGs)
dim(PredictAge)
cor(SelectCpGs$OriginalAge, SelectCpGs$PredictAge, method = "pearson") #Checking correlation # 0.6583068
...
write.csv(Age_Compare, 'Merged_tl_Diseased_AgeCompare.csv', row.names = F)
...
ggplot(Age_Compare, aes(OriginalAge, PredictAge)) +
  ...
  ggtitle("Age Comparison Plot - tl [r = 0.6583068]") +
